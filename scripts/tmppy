...
class InteractionProfile:
    ...
    def _fill_in_data(self,docked_dir, ensemble_dir, glob_pattern):
        ...
        docks = glob.glob(docked_dir + '/' + glob_pattern)
        docks
...
#    #    print str(r) + " - "  + str(freqs[r])
    ...
    if is_testing == False:
        ...
        os.system("cp centroids-box%s.pdb %s/" % (i+1,RESULT_DIR)) 
        os.system("cp com-box%s.pdb %s/" % (i+1,RESULT_DIR))
        os.system("cp NCvector-box%s.pdb %s/"% (i+1,RESULT_DIR))
